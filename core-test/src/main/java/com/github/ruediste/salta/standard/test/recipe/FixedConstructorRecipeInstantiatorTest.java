...
public class FixedConstructorRecipeInstantiatorTest {
	...
	public void testPublicConstructorInnerClass() {
		assertNotNull(injector.getInstance(PrivateTestClassPublicConstructor.class));
	}
	...
	public void testIsConstructorAccessible() throws Exception {
		checkIsConstructorAccessible(false, PrivateTestClassPublicConstructor.class);
		checkIsConstructorAccessible(true, PublicTestClassPublicConstructor.class);
		checkIsConstructorAccessible(false, TestClassPrivateConstructor.class);

	}

	private void checkIsConstructorAccessible(boolean expected, Class<?> clazz) throws NoSuchMethodException {
		assertEquals(expected,
				Accessibility.isConstructorAccessible(clazz.getDeclaredConstructor(), getClass().getClassLoader()));
	}
}
