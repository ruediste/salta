...
public abstract class MembersInjectorCreationRuleBase implements CreationRule {
    ...
    private <T> Consumer<T> getMembersInjector(TypeToken<T> typeLiteral, RecipeCreationContext ctx) {
    ...
    private <T> Consumer<T> getMembersInjectorNoCache(TypeToken<T> type, RecipeCreationContext ctx) {
        CompiledFunction function = ctx.getCompiler().compileFunction(getMembersInjectionRecipe(type, ctx));
        return new Consumer<T>() {
            ...
            public void accept(T t) {
                try {
                    ...
                    throw new SaltaException("Error while injecting members of instance of " + type, e);
                }
            }
    ...
    private FunctionRecipe getMembersInjectionRecipe(TypeToken<?> typeToken, RecipeCreationContext ctx) {
        FunctionRecipe recipe = membersInjectionRecipeCache.get(typeToken);
        if (recipe != null)
            return recipe;

        List<RecipeMembersInjector> injectors = config.construction.createRecipeMembersInjectors(ctx, typeToken);
        List<RecipeInitializer> initializers = config.construction.createInitializers(ctx, typeToken);
        ...
        return Optional.of(ctx -> {
            ...
            return new SupplierRecipe() {
                ...
                protected Class<?> compileImpl(GeneratorAdapter mv, MethodCompilationContext ctx) {
                    ...
                    ctx.addFieldAndLoad((Class) wrappedInjectorType, wrappedInjector);
