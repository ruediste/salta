...
public class CoreInjector {
     ...
     */
    public CoreInjector(CoreInjectorConfiguration config, List<CreationRule> creationRules) {
        this.config = config;
    ...
    public <T> Optional<T> tryGetInstance(CoreDependencyKey<T> key) {
        ...
        try {
            ...
            throw new SaltaException("Error while creating instance for " + key, e);
        }
        ...
        if (!tryGetCompiledRecipe.isPresent()) {
            ...
            if (rawType.getEnclosingClass() != null)
                throw new SaltaException(
                        "No instance found for inner class " + key + ".\nForgotten to make inner class static?");
    ...
    public Optional<CompiledSupplier> tryGetCompiledRecipe(CoreDependencyKey<?> key) {
        // use Double Checked Locking
        Optional<CompiledSupplier> compiledRecipe = compiledRecipeCache.get(key);
    ...
    public <T> T withRecipeCreationContext(Function<RecipeCreationContext, T> func) {
        RecipeCreationContextImpl ctx = new RecipeCreationContextImpl(CoreInjector.this);
    ...
    public SupplierRecipe getRecipe(CoreDependencyKey<?> key, RecipeCreationContext ctx) {
        Function<RecipeCreationContext, SupplierRecipe> recipeFunction = getFromOptional(tryGetRecipeFunc(key), key);
        SupplierRecipe result = recipeFunction.apply(ctx);
        if (result == null)
            throw new SaltaException("Error while creating recipe for " + key + ". The result of " + recipeFunction
    ...
    public Optional<SupplierRecipe> tryGetRecipe(CoreDependencyKey<?> key) {
        return withRecipeCreationContext(ctx -> tryGetRecipeFunc(key).map(f -> f.apply(ctx)));
    }

    /**
     * Get the recipe for a key
     */
    public Optional<Function<RecipeCreationContext, SupplierRecipe>> tryGetRecipeFunc(CoreDependencyKey<?> key) {
        // acquire the recipe lock
        synchronized (recipeLock) {
            Optional<Function<RecipeCreationContext, SupplierRecipe>> result = recipeCache.get(key);
            if (result == null) {
                try {
                    ...
                    throw new SaltaException("Error while creating recipe for " + key, t);
                }
            }
            return result;
        }
     ...
     * {@link #recipeLock}
     */
    private Optional<Function<RecipeCreationContext, SupplierRecipe>> createRecipe(CoreDependencyKey<?> key) {
        try {
            Optional<Function<RecipeCreationContext, SupplierRecipe>> recipeFunction = Optional.empty();
            // check rules
                        ...
                        public Function<RecipeCreationContext, SupplierRecipe> apply(
                            ...
                            return new Function<RecipeCreationContext, SupplierRecipe>() {
                                ...
                                public SupplierRecipe apply(RecipeCreationContext ctx) {
                                    return config.applyEnhancers(f.apply(ctx), ctx, key);
                                }
