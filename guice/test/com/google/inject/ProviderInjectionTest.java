...
public class ProviderInjectionTest extends TestCase {
	...
	public void testProvidersAreInjectedWhenBound() {
		...
		try {
			Guice.createInjector(m).getInstance(Bar.class);
	...
	public void testProvidersAreInjectedBeforeTheyAreUsed() {
		...
		assertEquals("Providers not injected before use", "[true]",
				injector.getInstance(String.class));
	}

	/**
	 * This test ensures that regardless of binding order, instances are
	 * injected before they are used. It injects mutable Count objects and
	 * records their value at the time that they're injected.
	 */
	public void testCreationTimeInjectionOrdering() {
		...
		assertEquals(28,
				injector.getInstance(Key.get(Count.class, named("c"))).value);
		assertEquals(30,
				injector.getInstance(Key.get(Count.class, named("b"))).value);
		assertEquals(31,
				injector.getInstance(Key.get(Count.class, named("a"))).value);
