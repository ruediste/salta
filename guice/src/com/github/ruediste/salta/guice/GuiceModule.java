...
public class GuiceModule implements Module {
	...
	public GuiceModule(GuiceInjectorConfiguration guiceConfig, GuiceInjectorImpl injector) {
	...
	public void configure(Binder binder) {
		...
		if (guiceConfig.requireExplicitBindings) {
			...
			for (TypeToken<?> type : guiceConfig.typesBoundToDefaultCreationRecipe) {
				...
				if (providedBy != null) {
					...
					binding.dependencyMatcher = DependencyKey.rawTypeMatcher(providedBy.value());
					binding.recipeFactory = ctx -> config.construction
							.createConcreteConstructionRecipe(TypeToken.of(providedBy.value()), ctx);
					binding.scopeSupplier = () -> config.scope.getScope(providedBy.value());
			...
			for (TypeToken<?> type : guiceConfig.typesBoundToDefaultCreationRecipe) {
				...
				if (implementedBy != null) {
					if (rawTypes.add(implementedBy.value())) {
						StandardStaticBinding binding = new StandardStaticBinding();
						binding.dependencyMatcher = DependencyKey.rawTypeMatcher(implementedBy.value());
						binding.recipeFactory = ctx -> config.construction
								.createConcreteConstructionRecipe(TypeToken.of(implementedBy.value()), ctx);
						binding.scopeSupplier = () -> config.scope.getScope(implementedBy.value());
						guiceConfig.automaticStaticBindings.add(binding);
					}
				}
			}
			...
			for (CoreDependencyKey<?> foo : guiceConfig.implicitlyBoundKeys) {
				if (keys.add(foo)) {
					...
					binding.recipeFactory = ctx -> config.construction.createConcreteConstructionRecipe(foo.getType(),
							ctx);
					binding.scopeSupplier = () -> config.scope.getScope(foo.getType());
					guiceConfig.automaticStaticBindings.add(binding);
				}
			}
		} else {
		...
		binder().getDelegate().bindScope(Singleton.class, config.singletonScope);
		binder().getDelegate().bindScope(javax.inject.Singleton.class, config.singletonScope);
		...
		config.membersInjectorFactory = new MembersInjectorFactory() {
			...
			public <T> Consumer<T> createMembersInjector(TypeToken<T> type) {
				MembersInjector<T> inner = injector.getMembersInjector(TypeLiteral.get(type));
		...
		config.creationPipeline.coreCreationRuleSuppliers.add(
		...
		config.creationPipeline.creationRules.add(new CreationRule() {
			...
			public Optional<Function<RecipeCreationContext, SupplierRecipe>> apply(CoreDependencyKey<?> key,
				...
				if (Stage.class.equals(key.getType().getType()))
					return Optional.of(ctx -> new SupplierRecipeImpl(() -> guiceConfig.stage));
				else
	...
	private void addProvidedByConstructionRule() {
		// rule for @ProvidedBy
		config.construction.constructionRules.add(new ProvidedByConstructionRuleBase(Provider.class) {

			@Override
			protected DependencyKey<?> getProviderKey(TypeToken<?> type) {
				ProvidedBy providedBy = type.getRawType().getAnnotation(ProvidedBy.class);
				if (providedBy != null) {
					if (!type.isSupertypeOf(
							TypeToken.of(providedBy.value()).resolveType(Provider.class.getTypeParameters()[0]))) {
						throw new SaltaException("Provider " + providedBy.value()
								+ " specified by @ProvidedBy does not provide " + type);
					}
				...
				while (!Object.class.equals(cls)) {
					for (Method m : cls.getDeclaredMethods()) {
						...
						if (idx.isOverridden(m)) {
							throw new SaltaException("Overriding @Provides methods is not allowed:\n " + m
									+ "\nis overridden by \n " + idx.getOverridingMethods(m).stream()
											.map(java.util.Objects::toString).collect(joining("\n ")));
						}
					}
		...
		ProviderMethodBinder b = new ProviderMethodBinder(config) {
			...
			protected boolean isProviderMethod(Method m) {
				...
				if (void.class.equals(m.getReturnType())) {
					throw new SaltaException("@Provides method may not return void:\n" + m);
				}
	...
	private void addStaticMembersInjectionDynamicInitializer() {
		...
		config.dynamicInitializers.add(() -> new StaticMembersInjectorBase() {
			...
			protected InjectionInstruction shouldInject(Method method) {
				...
				javax.inject.Inject inject = method.getAnnotation(javax.inject.Inject.class);
			...
			protected InjectionInstruction shouldInject(Field field) {
				...
				javax.inject.Inject inject = field.getAnnotation(javax.inject.Inject.class);
				if (guiceInject == null && inject == null) {
					return InjectionInstruction.NO_INJECT;
				}
	...
	private void addEagerInstantiationDynamicInitializer() {
		config.dynamicInitializers.add(() -> {
			injector.getSaltaInjector().getCoreInjector().withRecipeCreationContext(ctx -> {
		...
		config.construction.instantiatorRules.add(new ConstructorInstantiatorRuleBase(config) {
			...
			public Optional<Function<RecipeCreationContext, RecipeInstantiator>> apply(TypeToken<?> typeToken) {
				...
				if (TypeLiteral.class.equals(typeToken.getType())) {
					throw new SaltaException("Cannot inject a TypeLiteral that has no type parameter");
				}
			...
			protected Integer getConstructorPriority(Constructor<?> c) {
				...
				if (guiceInject != null && guiceInject.optional()) {
					throw new SaltaException(
							c + " is annotated @Inject(optional=true), but constructors cannot be optional");
				}
			...
			protected DependencyKey<?> getImplementorKey(TypeToken<?> type) {
				...
				if (implementedBy != null) {
					...
					if (type.getRawType().equals(implementedBy.value())) {
						throw new SaltaException("@ImplementedBy points to the same class it annotates. type: " + type);
					}
	...
	private void addTypeLiteralCreationRule() {
		...
		config.creationPipeline.creationRules
				.add(new CreationRuleImpl(k -> TypeLiteral.class.equals(k.getRawType()), key -> {
					TypeToken<?> type = key.getType();
					if (type.getType() instanceof Class) {
						throw new SaltaException("Cannot inject a TypeLiteral that has no type parameter");
					}
					TypeToken<?> typeParameter = type.resolveType(TypeLiteral.class.getTypeParameters()[0]);
					...
					TypeLiteral<?> typeLiteral = TypeLiteral.get(typeParameter.getType());
					return () -> typeLiteral;
				}));
	}
		...
		config.requiredQualifierExtractors.add(annotatedElement -> {
			return Arrays.stream(annotatedElement.getAnnotations())
					.filter(a -> a.annotationType().isAnnotationPresent(BindingAnnotation.class)
							|| a.annotationType().isAnnotationPresent(javax.inject.Qualifier.class));
		});

		config.availableQualifierExtractors.add(new Function<AnnotatedElement, Stream<Annotation>>() {
			@Override
		...
		config.creationPipeline.creationRules.add(new MembersInjectorCreationRuleBase(config) {
			...
			protected TypeToken<?> getDependency(CoreDependencyKey<?> key) {
				...
				if (MembersInjector.class.equals(key.getRawType())) {
					...
					TypeToken<?> dependency = key.getType().resolveType(rawType.getTypeParameters()[0]);
					return dependency;
				} else
					return null;
			}
	...
	private void addProviderCrationRules() {
		...
		config.creationPipeline.creationRules
				.add(new ProviderCreationRule(key -> key.getRawType().equals(Provider.class),
						(type, supplier) -> (Provider<Object>) () -> supplier.get(), Provider.class));
		config.creationPipeline.creationRules.add(new ProviderCreationRule(
				key -> key.getRawType().equals(javax.inject.Provider.class),
				(type, supplier) -> (javax.inject.Provider<Object>) () -> supplier.get(), javax.inject.Provider.class));
	}
	...
	private void addQualifierMatchingRules() {
		...
		config.qualifierMatchingAnnotationRules.add(new BiFunction<Annotation, Annotation, Boolean>() {
			@Override
			public Boolean apply(Annotation required, Annotation available) {
				if (required instanceof javax.inject.Named && available instanceof Named) {
					return Objects.equal(((javax.inject.Named) required).value(), ((Named) available).value());
				}
				if (available instanceof javax.inject.Named && required instanceof Named) {
					return Objects.equal(((Named) required).value(), ((javax.inject.Named) available).value());
				}
				return null;
			}
		});

		config.qualifierMatchingTypeRules.add(new BiFunction<Annotation, Class<?>, Boolean>() {

			@Override
			public Boolean apply(Annotation required, Class<?> availableType) {
				if (Named.class.equals(availableType) || javax.inject.Named.class.equals(availableType)) {
					return Named.class.equals(required.annotationType())
							|| javax.inject.Named.class.equals(required.annotationType());
				}
				return null;
			}
		});
				...
				if (Logger.class.equals(rawType)) {
					return Optional.of(ctx -> {
						if (key instanceof InjectionPoint) {
							...
							return new SupplierRecipeImpl(() -> Logger.getLogger(member.getDeclaringClass().getName()));

						}
						return new SupplierRecipeImpl(() -> Logger.getAnonymousLogger());
					});
				}
				return Optional.empty();
			}
