 ...
 *
 * <p>
 * {@code Key} supports generic types via subclassing just like
 * {@link TypeLiteral}.
 *
 * <p>
 * Keys do not differentiate between primitive types (int, char, etc.) and their
 * corresponding wrapper types (Integer, Character, etc.). Primitive types will
 * be replaced with their wrapper types when keys are created.
 *
...
public class Key<T> {
    ...
    /**
     * @return a {@link Supplier} which memoizes the value for lazy
     *         initialization.
     */
    private Supplier<String> createToStringSupplier() {
        // The performance hit on access is acceptable since the intended use is
        // for non-performance-
        // critical applications such as debugging and logging.
        return Suppliers.memoize(new Supplier<String>() {
            @Override
            public String get() {
                return "Key[type=" + typeLiteral + ", annotation="
                        + annotationStrategy + "]";
            }
        });
    }
